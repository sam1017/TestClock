...
public final class AlarmStateManager extends BroadcastReceiver {
    ...
    public static void deleteAllInstances(Context context, long alarmId,boolean manual) {
        LogUtils.i("deleteAllInstances: manual= " + manual);
        if (!manual) {
            deleteAllInstances(context, alarmId);
            return;
        }
        LogUtils.i("Deleting all instances of alarm: " + alarmId);
        ContentResolver cr = context.getContentResolver();
        List<AlarmInstance> instances = AlarmInstance.getInstancesByAlarmId(cr, alarmId);
        for (AlarmInstance instance : instances) {
            unregisterInstance(context, instance, true);
            AlarmInstance.deleteInstance(context.getContentResolver(), instance.mId,true);
        }
        updateNextAlarm(context);
    }
    public static void unregisterInstance(Context context, AlarmInstance instance, boolean manual) {
        LogUtils.i("unregisterInstance: manual= " + manual);
        if (!manual) {
            unregisterInstance(context, instance);
            return;
        }
        LogUtils.i("unregisterInstance  instance " + instance.mId);
        // Stop alarm if this instance is firing it
        AlarmService.stopAlarm(context, instance);
        AlarmNotifications.clearNotification(context, instance);
        cancelScheduledInstanceStateChange(context, instance);
        setDismissState(context, instance,manual);
    }
    public static void setDismissState(Context context, AlarmInstance instance,boolean manual) {
        LogUtils.i("setDismissState: manual= " + manual);
        if (!manual) {
            setDismissState(context, instance);
            return;
        }
        ...
        instance.mAlarmState = AlarmInstance.DISMISSED_STATE;
        final ContentResolver contentResolver = context.getContentResolver();
        AlarmInstance.updateInstance(contentResolver, instance, manual);
    }
    //bv zhangjimeng only update silent state use manual value, otherwise let it notify dataset change
    public static void registerInstance(Context context, AlarmInstance instance,
                                        boolean updateNextAlarm, boolean manual) {
        LogUtils.i("registerInstance: manual= " + manual);
        if (!manual) {
            registerInstance(context, instance,updateNextAlarm);
            return;
        }
        LogUtils.i("Registering instance: " + instance.mId);
        final ContentResolver cr = context.getContentResolver();
        ...
        if (currentTime.after(missedTTL)) {
            ...
            // Alarm is still active, so initialize as a silent alarm
            setSilentState(context, instance, manual);
        }
    ...
    public static void setSilentState(Context context, AlarmInstance instance, boolean manual) {
        LogUtils.i("setSilentState: manual= " + manual);
        if (!manual) {
            setSilentState(context, instance);
            return;
        }
