...
//bv zhangjiachu add for custom toast 20200520 start
...
public class AlarmUtils {
    /*bv zhangjiachu add for alarm style start */
    public static final int MSG_ADD_ALARM_ALTER =10401;

    /*bv zhangjiachu modify for alarm style: add handler start*/
    public static final int MSG_WORLD_CLOCK_ALTER =10771;
    ...
    /* bv zhangjiachu add for Alarm style start*/
    //bv zhangjiachu modify for custom toast 20200520 start
    /*public static void BvpopAlarmSetToast(Context context, long alarmTime) {
        final long alarmTimeDelta = alarmTime - System.currentTimeMillis();
        final String text = formatElapsedTimeUntilAlarm(context, alarmTimeDelta);
        Toast toast = Toast.makeText(context, text, Toast.LENGTH_LONG);
        ToastManager.setToast(toast);
    ...
    public static void BvpopAlarmSetToast(Context context, long alarmTime) {
        ...
        View layout = inflater.inflate(R.layout.bv_custom_toast,
                null);
        final TextView text = (TextView) layout.findViewById(R.id.toast_text);
        final long alarmTimeDelta = alarmTime - System.currentTimeMillis();
        final String textString = formatElapsedTimeUntilAlarm(context, alarmTimeDelta);
        text.setText(textString);
        Toast toast = new Toast(context);
        WindowManager windowManager = (WindowManager) context.getSystemService(WINDOW_SERVICE);
        Point size = new Point();
        windowManager.getDefaultDisplay().getSize(size);
        Log.d("bvtest", "BvpopAlarmSetToast: size.y*10/65 = " + size.y*10/65 + "   size.y = " + size.y);
        toast.setGravity(Gravity.BOTTOM, 0, size.y*10/65);
        toast.setDuration(Toast.LENGTH_SHORT);
        toast.setView(layout);
        toast.show();
    }
    //bv zhangjiachu modify for custom toast 20200520 end
    /* bv zhangjiachu add for Alarm style end*/
