...
//bv zhangjiachu modify for alarm style 20200525 start
import android.graphics.Color;
import android.graphics.PorterDuff;
import android.graphics.drawable.Drawable;
//bv zhangjiachu modify for alarm style 20200525 end
...
public final class TimerFragment extends DeskClockFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        final View view = inflater.inflate(Utils.isBvOS()? R.layout.bv_timer_fragment
    ...
    public void onResume() {
        ...
        if (intent != null && intent.hasExtra(TimerService.EXTRA_TIMER_ID)) {
            ...
            if (timer != null) {
                ...
                final int index = DataModel.getDataModel().getTimers().indexOf(timer);
                mViewPager.setCurrentItem(index);
                //bv zhangjiachu modify for no need to set visibility gone 20210306 start
                animateToView(mTimersView, null, false);
                //bv zhangjiachu modify for no need to set visibility gone 20210306 end
            }
        }
    }
    ...
    private void updateFab(@NonNull ImageView fab, boolean animate) {
        if (mCurrentView == mTimersView) {
            ...
            //bv zhangjiachu add for alarm style 20200721 start
            fab.setAlpha(1.0f);
            fab.setClickable(true);
            //bv zhangjiachu add for alarm style 20200721 end
            switch (timer.getState()) {
                ...
                case RESET:
                    if (Utils.isBvOS()){
                        if (animate) {
                            fab.setImageResource(R.drawable.ic_bv_stop_play_animation);
                        } else {
                ...
                case EXPIRED:
                    if (Utils.isBvOS()){
                        fab.setImageResource(R.drawable.ic_bv_stop_white_24dp);
                    } else {
            ...
            if (mCreateTimerView.hasValidInput()) {
                if (Utils.isBvOS()){
                    fab.setImageResource(R.drawable.ic_bv_start_white_24dp);
                } else {
                ...
                //bv zhangjiachu modify for alarm style 20200721 start
                if (Utils.isBvOS()){
                    fab.setImageResource(R.drawable.ic_bv_start_white_24dp);
                } else {
                    fab.setImageResource(R.drawable.ic_start_white_24dp);
    ...
    public void onUpdateTabs(@NonNull TabLayout tabs, @NonNull LinearLayout delete_alarm_tab, @NonNull TextView deskTitle) {
        deskTitle.setText(R.string.menu_timer);
    ...
    public void onUpdateFabButtons(@NonNull ImageButton left, @NonNull ImageButton right) {
        ...
        if (!isAdded()) return;  //bv zhangjiachu add for fixbug:monkey测试报错
        final Resources resources = getResources();
        if (mCurrentView == mTimersView) {
            left.setClickable(true);
            Drawable icDelete = resources.getDrawable(R.drawable.ic_bv_delete);
            Drawable icAddStopWatch = resources.getDrawable(R.drawable.ic_add);
            if(ThemeUtils.isDarkTheme(getActivity())){
                icDelete.setColorFilter(Color.WHITE, PorterDuff.Mode.SRC_ATOP);
                icAddStopWatch.setColorFilter(Color.WHITE, PorterDuff.Mode.SRC_ATOP);
            }
            left.setImageDrawable(icDelete);
            right.setImageDrawable(icAddStopWatch);
            ...
            Drawable icCancel = resources.getDrawable(R.drawable.ic_cancel);
            if(ThemeUtils.isDarkTheme(getActivity())){
                icCancel.setColorFilter(Color.WHITE, PorterDuff.Mode.SRC_ATOP);
            }
            left.setImageDrawable(icCancel);
            //bv zhangjiachu add for modify Image color in dark theme 20200528 start
            /*bv zhangjiachu modify for alarm style 20200103 end*/
    ...
    private void showCreateTimerView(int updateTypes) {
        ...
        //bv zhangjiachu modify for no need to set visibility gone 20210306
    ...
    private void animateToView(final View toView, final Timer timerToRemove,
        ...
        viewTreeObserver.addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
            ...
            public boolean onPreDraw() {
                ...
                if (DEBUG) {
                    Log.d(TAG, "onPreDraw: mCurrentView == mTimersView = " + (mCurrentView == mTimersView));
                }
    ...
    private Timer getTimer() {
        ...
        //bv zhangjiachu modify for fixbug：2189 20210202 start
        ...
        return mAdapter.getCount() == 0 ? null : mAdapter.getTimer(index);
        //bv zhangjiachu modify for fixbug：2189 20210202 end
    }
