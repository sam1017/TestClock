...
public final class Alarm implements Parcelable, ClockContract.AlarmsColumns {
    ...
    public static boolean updateAlarm(ContentResolver contentResolver, Alarm alarm, boolean manual) {
        if (!manual) {
            return updateAlarm(contentResolver, alarm);
        }
        ...
        Uri uri = getContentUri(alarm.id);
    ...
    public static boolean deleteAlarm(ContentResolver contentResolver, long alarmId,boolean manual)  {
        if (!manual) {
            return deleteAlarm(contentResolver, alarmId);
        }
        if (alarmId == INVALID_ID) return false;
        // should call manual method,
        Uri uri = getContentUri(alarmId);
