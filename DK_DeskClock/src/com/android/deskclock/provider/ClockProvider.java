...
public class ClockProvider extends ContentProvider {
    ...
    /*bv zhangjimeng 2020/07/08 add for lowlevel phone performance, begin*/
    public int update(@NonNull Uri uri, ContentValues values, String where, String[] whereArgs, boolean manual) {
        if (!manual) {
            return update(uri, values, where, whereArgs);
        }
        int count;
        String alarmId;
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
    ...
    public Uri insert(@NonNull Uri uri, ContentValues initialValues, boolean manual) {
        if (!manual) {
            return insert(uri, initialValues);
        }
        long rowId;
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        switch (sURIMatcher.match(uri)) {
    ...
    public int delete(@NonNull Uri uri, String where, String[] whereArgs, boolean manual) {
        if (!manual) {
            return delete(uri, where, whereArgs);
        }
        int count;
        String primaryKey;
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
    ...
    public Bundle call(String method, String arg, Bundle extras) {
        ...
        if (extras != null) {
            values = (ContentValues) (extras.getParcelable("values"));
            where = extras.getString("where");
            whereArgs = extras.getStringArray("whereArgs");
        }
        if (arg != null) {
            uri = Uri.parse(arg);
        }
        ...
        switch (method) {
            case ProviderConstant.CALL_METHOD_UPDATE:

                update(Uri.parse(arg), values, where, whereArgs, true);
                break;
            case ProviderConstant.CALL_METHOD_INSERT:
                Uri uriResult = insert(uri, values, true);
                result.putString("uriResult",uriResult.toString());
                return result;
            case ProviderConstant.CALL_METHOD_DELETE:
                delete(Uri.parse(arg), where, whereArgs, true);
                break;
        }
        return null;
    }
