...
import src.com.android.deskclock.provider.ProviderConstant;
...
public final class AlarmInstance implements ClockContract.InstancesColumns {
    ...
    public static AlarmInstance addInstance(ContentResolver contentResolver,
                                            AlarmInstance instance, boolean manual) {
        LogUtils.i("addInstance manual" + manual);
        if (!manual) {
            return addInstance(contentResolver, instance);
        }
        ...
        for (AlarmInstance otherInstances : getInstances(contentResolver, dupSelector)) {
            if (otherInstances.getAlarmTime().equals(instance.getAlarmTime())) {
                ...
                // Copy over the new instance values and update the db
                instance.mId = otherInstances.mId;
                updateInstance(contentResolver, instance, manual);
                return instance;
            }
        }
        ...
        Bundle result = contentResolver.call(uri, ProviderConstant.CALL_METHOD_INSERT, uri.toString(), bundle);
        if (result != null) {
            String str = result.getString("uriResult");
            if (str != null) {
                instance.mId = getId(Uri.parse(str));
            }
        }
        return instance;
    }

    public static boolean updateInstance(ContentResolver contentResolver, AlarmInstance instance, boolean manual) {
        if (!manual) {
            return updateInstance(contentResolver, instance);
        }
        if (instance.mId == INVALID_ID) return false;
        ContentValues values = createContentValues(instance);
        // should call manual method,
        Uri uri =getContentUri(instance.mId);
        Bundle bundle=new Bundle();
        bundle.putParcelable("values",values);
        contentResolver.call(uri, ProviderConstant.CALL_METHOD_UPDATE, uri.toString(), bundle);
        return true;
    }

    public static boolean deleteInstance(ContentResolver contentResolver, long instanceId, boolean manual) {
        if (!manual) {
            return deleteInstance(contentResolver, instanceId);
        }
        if (instanceId == INVALID_ID) return false;

        // should call manual method,
        Uri uri = getContentUri(instanceId);
        contentResolver.call(uri, ProviderConstant.CALL_METHOD_DELETE, uri.toString(), null);
        return true;
    }
