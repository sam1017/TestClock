...
public class RingtonePickerActivity extends BaseActivity
    ...
    public static Alarm BvTempAlarm = new Alarm();
    /*bv zhangjiachu add for alarm style*/
    /**
     ...
     */
    static AlarmUpdateCallBack alarmUpdateCallBack;
    public static void setCallBack(Intent intent, AlarmUpdateCallBack callBack, Context context){
        alarmUpdateCallBack = callBack;
    }
    ...
    protected void onPause() {
        ...
        /*bv zhangjiachu add for alarm style start*/
        if(Utils.isBvOS()){
            //bv zhangjiachu add for: 新建时钟选择闹钟关联 20210203 start
            if(mHandler != null) {
                BvTempAlarm.alert = mSelectedRingtoneUri;
                Message msg = mHandler.obtainMessage();
                msg.what = AlarmUtils.MSG_ADD_ALARM_ALTER;
                msg.obj = BvTempAlarm;
                mHandler.sendMessage(msg);
            }
            //bv zhangjiachu add for: 新建时钟选择闹钟关联 20210203 end
            if (mSelectedRingtoneUri != null) {
                if (mAlarmId != -1) {
                    final Context context = getApplicationContext();
                    ...
                    new AsyncTask<Void, Void, Alarm>() {
                        ...
                        protected void onPostExecute(Alarm alarm) {
                            ...
                            if (alarm != null) {
                                ...
                                AlarmUpdateHandler alarmUpdateHandler = new AlarmUpdateHandler(context, null, null);
                                alarmUpdateHandler.setAlarmUpdateCallBack(alarmUpdateCallBack);
        ...
        }else {/*bv zhangjiachu add for alarm style end*/
            if (mSelectedRingtoneUri != null) {
                if (mAlarmId != -1) {
                    final Context context = getApplicationContext();
                    final ContentResolver cr = getContentResolver();
